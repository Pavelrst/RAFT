...
class Meshgrid(nn.Module):
    ...
    def _compute_meshgrid(self, width, height):
        self.rangex = torch.arange(0, width)
        self.rangey = torch.arange(0, height)
...
class BatchSub2Ind(nn.Module):
    ...
    def forward(self, shape, row_sub, col_sub, out=None):
        ...
        self._offsets = torch.arange(batch_size)
...
class Interp2(nn.Module):
    ...
    def forward(self, v, xq, yq):
        ...
        # batch_sub2ind
        self._i00 = self._batch_sub2ind([height, width], self._y0, self._x0)
        self._i01 = self._batch_sub2ind([height, width], self._y0, self._x1)
        self._i10 = self._batch_sub2ind([height, width], self._y1, self._x0)
        self._i11 = self._batch_sub2ind([height, width], self._y1, self._x1)

        # reshape
        v_flat = _bchw2bhwc(v).contiguous().view(-1, channels)
        self._v00 = torch.index_select(v_flat, dim=0, index=self._i00.view(-1))
        self._v01 = torch.index_select(v_flat, dim=0, index=self._i01.view(-1))
        self._v10 = torch.index_select(v_flat, dim=0, index=self._i10.view(-1))
        self._v11 = torch.index_select(v_flat, dim=0, index=self._i11.view(-1))

        # local_coords
        self._x = torch.add(xq, - self._x0.float())
        self._y = torch.add(yq, - self._y0.float())
